...
# main ve veritabanı modülleri yüklenmeden önce test ortamı için geçici SQLite adresi atanıo
...
def test_scan_url_success(mocker):
    ...
    """
    # Ağ isteği yapmaması ve takılmaması için extract_features fonksiyonunu mockluyoruz
    sahte_ozellikler = {
        "url_uzunlugu": 19,
...
def test_scan_url_invalid_url():
    ...
    """
    # Şemandaki check_whitespace validator'ü boşluk algılayıp hata fırlatmalı
...
def test_scan_url_db_error_and_rollback(mocker):
    ...
    """
    # 1. extract_features başarılı geçsin diye mockluyoruz
    mocker.patch("main.extract_features", return_value={"test_feature": 1})

    # 2. SQLAlchemy veritabanı oturumunun commit ve rollback metodlarını mockluyoruz
    ...
    # Hatayı yakalayıp 500 dönmeli
    assert response.status_code == 500
    assert "Veritabanı kayıt hatası" in response.json()["detail"]

    # Commit çağrılmış ve patlamış olmalı
    mock_commit.assert_called_once()
    # En kritik nokta: Hata durumunda db.rollback() kesin olarak çalıştırılmış mı?
